    ...
    void makeModule(core::Context ctx, ImportTree *node, vector<core::NameRef> &parts, ast::ClassDef::RHS_store &modRhs,
        ...
        if (source.exists()) {
            ...
            if (moduleType != ModuleType::Private || source.isNormalImport()) {
                ...
                const auto &moduleLoc = [](auto &source, auto &moduleType, auto packageLoc) {
                    switch (moduleType) {
                        case ModuleType::Private:
                            // only normal imports in this case
                            return source.isEnumeratedImport ? source.importLoc : core::LocOffsets::none();
                        case ModuleType::PrivateTest: {
                            // can have all 3 kinds of imports in this case
                        ...
                        case ModuleType::PublicTest:
                            // always friend imports in this case
                            return packageLoc->offsets();
                        default:
                            return core::LocOffsets::none();
                    }
                }(source, moduleType, packageLoc);
